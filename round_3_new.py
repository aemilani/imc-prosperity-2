...
class Trader:
    def run(self, state: TradingState):
        ...
        if ('CHOCOLATE' in state.order_depths) and ('STRAWBERRIES' in state.order_depths) and\
            ...
            diff_mean = 379.5
            diff_std = 76.4
            fair_diff_high = diff_mean + 0.2 * diff_std
            fair_diff_low = diff_mean - 0.2 * diff_std
            thr_high = diff_mean + 0.5 * diff_std
            thr_low = diff_mean - 0.5 * diff_std
            ...
            elif fair_diff_high < diff < fair_diff_high:  # Liquidate positions
